...
def build_walls_array(idf, model, runner, user_arguments, sql)
	...
	model.getSurfaces.each do |surf|
		...
		#runner.registerInfo("Wall Construction is #{const}.")
		# Get the area
		...
		if surf.outsideBoundaryCondition == 'Surface'
			...
			#runner.registerInfo("#{wall_name} has an outside boundary condition object name #{shared_wall}. Must check if its already duplicated.")
		...
		if duplicate == 0 and surf.outsideBoundaryCondition == 'Surface'
			shared_surface_walls_completed << wall_name
			#runner.registerInfo("#{wall_name} was added to list of completed walls.")
		...
		#find wall height
		wall_z_max = -1000000000
		wall_z_min = 1000000000
		...
		layers.each_with_index do |layer, i|
			...
			if i == 0
				...
				#runner.registerInfo("interior layer standards info = #{int_layer}.")
		...
		#runner.registerInfo("Interior Layer Identifier (String) = #{il_identifier}.")
		#runner.registerInfo("Exterior Layer Identifier (String) = #{el_identifier}.")
		...
		if category.to_s.include?('Wood Framed')
			...
			layers.each_with_index do |layer, i|
				...
				if i > sl_i
					...
					if int_frame_stds.standardsCategory.is_initialized
						...
						if int_frame_category.include?('Framed')
							...
							if int_frame_stds.compositeFramingMaterial.is_initialized
								...
								if int_frame_mat == 'METAL'
									int_wall_type = 'STEEL_FRAME'
								else
							...
							if int_frame_stds.compositeCavityInsulation.is_initialized
								...
								if int_ins_r > 0 and int_ins_thickness > 0
							...
							if int_frame_stds.standardsIdentifier.is_initialized and int_ins_r > 0 and int_ins_thickness > 0
								...
								if int_frame_identifier.include?('cellulose')
									...
									if int_ins_r > 5
										int_ins_mat = 'SPRAY_FOAM_CLOSED_CELL'
									elsif int_ins_r < 5
										int_ins_mat = 'SPRAY_FOAM_OPEN_CELL'
									else
										int_ins_mat = 'SPRAY_FOAM_UNKNOWN'
									end
								else
									int_ins_mat = 'BATT_FIBERGLASS'
								end
							elsif int_ins_r > 0 and int_ins_thickness > 0
								int_ins_mat = 'BATT_FIBERGLASS'
							else
								int_ins_mat = nil
							end
					...
					else	# Default to nil (no framing found)
						int_frame_mat = nil
						int_on_center_in = nil
						int_studs_size = nil
						int_ins_mat = nil
		...
		# SIPS Wall Type
			...
			elsif sips_thickness == '8 1/4'
				cav_thickness = (8.25 - 0.875)
		...
		surf.subSurfaces.each do |ss|
			...
			if (subsurface_type == 'FixedWindow') || (subsurface_type == 'OperableWindow')
				#Determine operability
				if subsurface_type == 'FixedWindow'
					operable = false
				elsif subsurface_type == 'OperableWindow'
					operable = true
				else
					operable = false
				end
				...
				window_z_max = -1000000000
				window_z_min = 1000000000
			...
			#Question: Can we use the U value to guess the material?
				...
				door_z_max = -1000000000
				door_z_min = 1000000000
				#runner.registerInfo("finding subsurface vertices.")
				vertices = ss.vertices
				#runner.registerInfo("found subsurface vertices.")
				vertices.each do |vertex|
...
def build_foundation_walls_array(idf, model, runner, user_arguments, sql)
	...
	model.getSurfaces.each do |surf|
		...
		else								# Includes metal and brick walls.
			wall_type = 'OTHER_WALL_TYPE'
			# define the framing size; there are no studs for SIPs
