...
def compressive_strength_enum(compressive_strength_value)
...
def build_roofs_array(idf, model, runner, user_arguments, sql)
  ...
  model.getSurfaces.each do |surf|
    ...
    next if surf.surfaceType != 'RoofCeiling'
    ...
    next if !construction_stds.intendedSurfaceType.to_s.include?('Roof')
    ...
    if construction_stds.intendedSurfaceType.is_initialized
      ...
      exterior_adjacent_to = case surface_type
                             when 'ExteriorRoof'
                               'AMBIENT'
                             when 'AtticRoof'
                               'AMBIENT'
                             ...
                             when 'GroundContactRoof'
                               ...
                               'OTHER_EXTERIOR_ADJACENT_TO'
                             end
    else
      case surf.outsideBoundaryCondition
      when 'Outdoors'
        exterior_adjacent_to = 'AMBIENT'
      when 'Ground', 'Foundation'
        exterior_adjacent_to = 'GROUND'
      when 'Zone', 'Adiabatic', 'Surface'
    ...
    # find roof width
    roof_z_max = -1_000_000_000
    roof_z_min = 1_000_000_000
    ...
    if !il_identifier.nil?
      if il_identifier.include?('Gypsum Board - 1/2 in.') || il_identifier.include?('Gypsum Board - 3/8 in.')
        interior_wall_finish = 'GYPSUM_REGULAR_1_2'
      elsif il_identifier.include?('Gypsum Board - 3/4 in.') || il_identifier.include?('Gypsum Board - 5/8 in.')
    ...
    if !el_identifier.nil?
      ...
      elsif el_identifier.include?('Asphalt') && el_identifier.include?('Shingle')
        roof_type = 'ASPHALT_OR_FIBERGLASS_SHINGLES'
      elsif el_identifier.include?('Wood Shingles') || el_identifier.include?('Woods')
      ...
      elsif el_identifier.include?('tile') || el_identifier.include?('Tile') || el_identifier.include?('Slate')
    ...
    radiant_barrier = if (air_barrier == true) || (!vapor_barrier.nil?)
                        true
                      else
                        false
                      end
    ...
    if category.to_s.include?('Wood Framed') # Should only be Wood Framed Rafter Roof
      ...
      ins_mat = if !ins_r_value_per_in.nil?
    ...
    elsif category.to_s.include?('Concrete') && (!category.to_s.include?('Sandwich Panel'))
      ...
      compressive_strength = compressive_strength_enum(compressive_strength_value)
    ...
    elsif category.to_s.include?('CLT') || category.to_s.include?('Cross Laminated Timber') || category.to_s.include?('Woods') # not a tag option at the moment
      ...
      # runner.registerInfo("Structural Layer Identifier = #{identifier}.")
      live_load = 50
      live_load = /(\d+)\spsf/.match(identifier).to_s.to_f if !category.nil?
    ...
    layers.each_with_index do |layer, i|
      # Skip fenestration, partition, and airwall materials
      next if (roof_decking_type == 'WOOD') || (roof_decking_type == 'METAL')
      ...
      next if !deck_stds.standardsIdentifier.is_initialized
      ...
      roof_decking_type = if deck_identifier.include?('osb') || deck_identifier.include?('plywood')
                            'WOOD'
    ...
    roof_construction_type = if rafters_material == 'WOOD_RAFTER'
                               'RAFTER' # Ignores TRUSS for now.
                             elsif rafters_material == 'METAL_RAFTER'
                               'RAFTER' # Ignores TRUSS for now.
                             elsif category.include?('Concrete') || category.include?('ICF')
                               'CONCRETE_DECK' #
                             else
                               # Handles all 'OTHER_MATERIAL'
                               'OTHER' # There is no rafter or truss for CLT, concrete, ICF, SIPs, or MIPs
                             end
    ...
    surf.subSurfaces.each do |ss|
      ...
      if subsurface_type == 'Skylight'
        ...
        skylight_z_max = -1_000_000_000
        skylight_z_min = 1_000_000_000
        ...
        vertices.each do |vertex|
          ...
          skylight_z_max = z if z > skylight_z_max
        end
        # runner.registerInfo("found max and min z vertices.")
        ...
        if sub_const_stds.fenestrationTint.is_initialized
          ...
          if os_low_e == true
            glass_type = 'LOW_E'
          elsif os_tint.include?('clear')
            glass_type = 'NONE_GLASS_TYPE'
          elsif os_tint.include?('tinted') || os_tint.include?('green') || os_tint.include?('blue') || os_tint.include?('grey') || os_tint.include?('bronze')
            glass_type = 'TINTED'
          else
            glass_type = 'NONE_GLASS_TYPE'
          end
        elsif !sub_const_stds.fenestrationTint.is_initialized
        ...
        if sub_const_stds.fenestrationGasFill.is_initialized
          ...
          gas_fill = if os_gas_fill.include?('air')
                       'AIR'
                     elsif os_gas_fill.include?('argon') || os_tint.include?('krypton')
                       'ARGON'
                     else
                       'NONE_GAS_FILL'
                     end
...
def get_attics(idf, model, runner, user_arguments, sql)
  ...
  model.getSpaces.each do |space|
    ...
    space.surfaces.each do |surf|
      ...
      if (surf.surfaceType == 'RoofCeiling') && construction_stds.intendedSurfaceType.to_s.include?('AtticRoof')
        ...
        roof_span_ft = OpenStudio.convert(roof_span_m, 'm', 'ft').get
        runner.registerInfo(" Roof surface span = #{roof_span_ft}.")
        attic_roof_span = roof_span_ft if roof_span_ft > attic_roof_span
      end
      next unless (surf.surfaceType == 'Floor') && construction_stds.intendedSurfaceType.to_s.include?('AtticFloor')
      # Skips everything but Roofs. Assumes construction_stds.intendedSurfaceType.is_initialized. Could error out if its not initialized...
      # space_is_attic = true
      ...
      attic_floor_span = floor_span_ft if floor_span_ft > attic_floor_span
    end
    # runner.registerInfo("Attic Roof list is: #{attic_roofs}.")
    # runner.registerInfo("Attic Floor list is: #{attic_floors}.")
    ...
    next unless (!attic_name.nil?) || (attic_roofs != [])
    attic = {
...
def get_atticandroof3(idf, model, runner, user_arguments, sql)
  ...
  model.getSpaces.each do |space|
    ...
    space.surfaces.each do |surf|
      ...
      if (surf.surfaceType == 'Wall') && construction_stds.intendedSurfaceType.to_s.include?('AtticWall')
      ...
      if (surf.surfaceType == 'RoofCeiling') && construction_stds.intendedSurfaceType.to_s.include?('AtticRoof')
        ...
        attic_roof_span = roof_span_ft if attic_roof_span < roof_span_ft
        ...
        if !el_identifier.nil?
          if el_identifier.include?('Metal') # Assumes metal is steel. Currently missing aluminum siding
            roof_type = 'METAL_SURFACING'
          elsif el_identifier.include?('Asphalt') && el_identifier.include?('shingles')
            roof_type = 'ASPHALT_OR_FIBERGLASS_SHINGLES'
          elsif el_identifier.include?('Wood Shingles') || el_identifier.include?('Woods')
            roof_type = 'WOOD_SHINGLES_OR_SHAKES'
          elsif el_identifier.include?('Shingles')
          ...
          if !ins_r_value_per_in.nil?
            if ins_r_value_per_in < 0.1
              ...
              if identifier.include?('glass')
                ...
                ins_mat = if ins_r_value_per_in < 5
      ...
      if !ins_r_value_per_in.nil?
        if ins_r_value_per_in < 0.1
          ...
          if identifier.include?('glass')
            ...
            cav_ins_mat = if ins_r_value_per_in < 5
                            'SPRAY_FOAM_OPEN_CELL'
                          elsif ins_r_value_per_in > 5
                            'SPRAY_FOAM_CLOSED_CELL'
                          else
