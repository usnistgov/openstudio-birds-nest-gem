...
class OpenStudio::Model::Construction
  ...
  def glass_u_factor
    ...
    unless self.isFenestration
      OpenStudio::logFree(
        ...
        "Cannot determine glass U-factor for #{name} because it is not a fenestration."
      )
      return 0.0
    end
    ...
    (u_factor.get if u_factor.is_initialized)
  end
  ...
  def glass_visible_transmittance
    ...
    unless self.isFenestration
      OpenStudio::logFree(
        OpenStudio::Warn,
        'openstudio.model.Construction',
        "Cannot determine glass visible transmittance for #{name} because it is not a fenestration."
      )
      return 0.0
    end
  ...
  def glass_solar_heat_gain_coefficient
    ...
    unless self.isFenestration
      OpenStudio::logFree(
        OpenStudio::Warn,
        'openstudio.model.Construction',
        "Cannot determine glass solar heat gain coefficient for #{name} because it is not a fenestration."
      )
      return 0.0
    end
    ...
    (shgc.get if shgc.is_initialized)
  end
  ...
  def rigid_insulation_r_value(struct_layer_i, side = 'interior')
    ### Why is side = interior here?
    ...
    possible_ins_layers.each do |layer|
      r_val_si += layer.thermalResistance if layer.nist_is_insulation
    end

    # Convert SI to IP
    OpenStudio.convert(r_val_si, 'm^2*K/W', 'ft^2*h*R/Btu').get if r_val_si.positive?
  end

  # Rigid Insulation Values (R and Thickness)
  def rigid_insulation_values(struct_layer_i, side = 'interior')
    ### Why is side = interior here?
    ...
    [r_val_ip, total_ins_thickness_ip]
  ...
  def row_id
    ...
    if row_id.is_initialized
      ...
      OpenStudio::logFree(
        OpenStudio::Warn,
        'openstudio.model.Construction',
        "Row ID not found for construction: #{name}.  Cannot find requested window property."
      )
      row_id = 9999
    end
