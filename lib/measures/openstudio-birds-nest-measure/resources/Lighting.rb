...
def light_wattage(light, idf, model, runner)
  ...
  thermal_zone = model.getThermalZones
                         ...
                         .last
  light_zone_area = thermal_zone.nil? ? 0 : thermal_zone.floorArea
...
def total_wattage(idf, model, runner)
  ...
  idf.getObjectsByType('Lights'.to_IddObjectType)
     .map { |light| light_wattage(light, idf, model, runner) }
     .sum
     .round(1)
end

def get_lighting(idf, pct_inc_lts, pct_mh_lts, pcf_cfl_lf_lts, pct_led_lts, model, runner)
  # Create Lighting Object. Only need the lighting group OR the lighting fraction with total wattage for BIRDS NEST.
  {
    ...
    'totalWattage' => total_wattage(idf, model, runner), # Aggregate wattage for all lighting objects in the OSM, currently defaulted to 999
    'ceilingFans' => [
      {
