...
def total_wattage(idf)
  ...
  idf.getObjectsByType('Lights'.to_IddObjectType)
     .map { |light| light_wattage(light, idf) }
     .sum
     .round(1)
end

def get_lighting(idf, pct_inc_lts, pct_mh_lts, pcf_cfl_lf_lts, pct_led_lts)
  # Create Lighting Object. Only need the lighting group OR the lighting fraction with total wattage for BIRDS NEST.
  {
    ...
    'totalWattage' => total_wattage(idf), # Aggregate wattage for all lighting objects in the OSM, currently defaulted to 999
    'ceilingFans' => [
      {
