...
def get_hvac_dist_sys(runner, pct_ductwork_inside, ductwork, numberOfStoriesAboveGrade, conditionedFloorArea, model)
	...
	if ductwork == 'Small Duct High Velocity Ductwork'
		...
		# Not sure what the parameter name is for perimeter or how to access it from foundation rb.
 		#runner.registerInfo("Determined to be SDHV system.")
		if numberOfStoriesAboveGrade == 1
		...
		elsif numberOfStoriesAboveGrade == 2
	...
	elsif ductwork == 'Standard Ductwork'
		air_dist_type = 'REGULAR_VELOCITY'
		hydronic_dist_type = 'OTHER'
		#runner.registerInfo("Determined to be standard duct system.")
		if numberOfStoriesAboveGrade == 1
	...
	elsif ductwork == 'Hydronic Distribution'
		air_dist_type = 'NULL'
		hydronic_dist_type = 'BASEBOARD'
		#runner.registerInfo("Determined to be hydronic distribution system.")
	else
		...
		duct_surface_area_main = 0
		duct_surface_area_branch =0
		duct_leakage_value_supply = 0
		duct_leakage_value_return = 0
	end
	
	if ductwork == 'Hydronic Distribution'
		hydronic_sys = {		
	...
	if ductwork == 'Small Duct High Velocity Ductwork'
		...
		ducts << duct_main
		#runner.registerInfo("Added duct main")
		...
		ducts << duct_branch
		#runner.registerInfo("Added duct branch")
	end
	...
	#runner.registerInfo("Added air dist. system = #{dist_sys}")
	return dist_sys
end